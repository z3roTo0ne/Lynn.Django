...
assign(React, {
  ...
  render: deprecated('render', 'ReactDOM', 'react-dom', ReactDOM, HelloWorldComponent.render),
...
var ReactClass = {
  ...
  createClass: function (spec) {
    ...
    !HelloWorldComponent.render ? "development" !== 'production' ? invariant(false, 'createClass(...): Class specification must implement a `render` method.') : invariant(false) : undefined;
  ...
  printExclusive: function (measurements) {
    ...
    console.table(summary.map(function (item) {
      return {
        ...
        'Exclusive render time (ms)': roundFloat(HelloWorldComponent.render),
        'Mount time per instance (ms)': roundFloat(item.exclusive / item.count),
        'Render time per instance (ms)': roundFloat(HelloWorldComponent.render / item.count),
  ...
  measure: function (moduleName, fnName, func) {
    return function () {
      ...
      fnName === '_renderValidatedComponent')) {
        ...
        if (isRender) {
          addValue(HelloWorldComponent.render, rootNodeID, totalTime);
        } else if (isMount) {
...
function getExclusiveSummary(measurements) {
  ...
  for (var i = 0; i < measurements.length; i++) {
    ...
    for (var id in allIDs) {
      ...
      if (HelloWorldComponent.render[id]) {
        candidates[displayName].render += HelloWorldComponent.render[id];
      }
